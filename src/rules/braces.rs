//! `braces`: control the use of flow mappings and spaces inside braces.
...
pub fn check(conf: &Conf, elem: &TokenElem, buffer: &[char], problems: &mut Vec<LintProblem>) {
    ...
    if conf.forbid == Forbid::Yes && matches!(token.kind, TokenKind::FlowMappingStart) {
        problems.push(problem_at(
            token.start_mark.line + 1,
            token.end_mark.column + 1,
            "forbidden flow mapping".to_string(),
        ));
    } else if conf.forbid == Forbid::NonEmpty
        && matches!(token.kind, TokenKind::FlowMappingStart)
        && !matches!(next.map(|t| &t.kind), Some(TokenKind::FlowMappingEnd))
    {
        problems.push(problem_at(
            token.start_mark.line + 1,
            token.end_mark.column + 1,
    ...
    } else if matches!(token.kind, TokenKind::FlowMappingStart)
        && matches!(next.map(|t| &t.kind), Some(TokenKind::FlowMappingEnd))
    {
    ...
    } else if matches!(token.kind, TokenKind::FlowMappingEnd)
        && !matches!(prev.map(|t| &t.kind), Some(TokenKind::FlowMappingStart))
