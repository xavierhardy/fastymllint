        ...
        for (idx, line) in ctx.lines.iter().enumerate() {
            ...
            while i < chars.len() {
                ...
                match c {
                    ...
                    ':' if !in_quote => {
                        // Check if it's a key colon (followed by space, newline, or end)
                        // In flow style it can be followed by anything except other flow markers maybe?
                        // yamllint seems to check all colons except those in quotes.
...
mod tests {
    ...
    fn test_colons_min_after() {
        ...
        let diagnostics = rule.check(&ctx, None);
        assert_eq!(diagnostics.len(), 1);
        assert_eq!(diagnostics[0].message, "too few spaces after colon");
    }
